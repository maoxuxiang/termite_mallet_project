...
DATASETS = [ DEFAULT_DATASET, 'try2', '20newsgroups', 'nsfgrants', 'nsf25k', 'nsf10k', 'nsf1k', 'poliblogs', 'gjp', 'fomc', 'CR_financial_collapse', 'CR_stock_market_plunge', 'FCIC_final_report', 'FCIC_first_hearing', 'FR_federal_open_market_committee', 'FR_monetary_policy_hearings' ]
...
def Demonstrate(dataset, model, is_quiet, force_overwrite):
	#database_folder = 'data/{}'.format(dataset)
	database_folder = 'data/{}/db'.format(dataset)
	corpus_folder = 'data/{}/db'.format(dataset)
	model_folder = 'data/{}/model-{}'.format(dataset, model)
	app_name = '{}_{}'.format(dataset, model)

	def PrepareDataset():
		executable = 'bin/read_mydataset.sh'
...
def main():
	...
	parser.add_argument( 'dataset'     , nargs = '?', type = str, default = DEFAULT_DATASET, help = 'Dataset identifier' )
